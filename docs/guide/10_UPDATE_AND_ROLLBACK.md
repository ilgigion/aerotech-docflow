...
```text
[1/14] Пакет распакован, manifest и preflight уже проверены.
[2/14] Повторная проверка NAPS2 и scanner lock после подтверждения...
[3/14] Отправка команды остановки Windows-службы...
[4/14] Проверка остановки aerotech-docflow.exe...
[5/14] Контрольная проверка сканера и рабочего WinSW XML...
[6/14] Перемещение старой установки в rollback...
[7/14] Перемещение файлов новой версии в Program Files...
[8/14] Восстановление рабочего WinSW XML без изменений...
[9/14] Проверка путей WinSW, XML и учётной записи службы...
[10/14] Вызов sc.exe start AerotechDocflow...
[11/14] Windows Service Control Manager подтвердил RUNNING.
[12/14] Проверка процессов WinSW и aerotech-docflow.exe...
[13/14] Ожидание локального /health новой версии...
[14/14] Очистка rollback, ZIP и временных файлов...
```

Во время ожидания дополнительно выводятся состояние SCM и номер каждой попытки
`/health`, поэтому длительная операция не выглядит как зависание. Перед запуском
и при ошибке в `updater.log` сохраняются фактические пути WinSW/XML, поля
`executable`, `arguments`, `workingdirectory`, `logpath`, SCM `StartName`, список
процессов, результат `sc.exe queryex` и хвост последнего wrapper-лога.
